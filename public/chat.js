...
socket.on("chat message", function (msg) {
	...
	if (msg.split("𐤟")[1].substring(1, 2) == "/") {
		...
		if (
			msg.split("𐤟")[1].substring(2, 3) == "i" &&
			msg.split("𐤟")[1].substring(3, 4) == " "
		) {
			temp =
				msg.split(" 𐤟 ")[0] +
				" 𐤟 " +
				'<img width="200" height="auto" src="' +
				msg.split("𐤟")[1].substring(4, msg.length) +
				'">';
			msg = temp;
		}
		...
		// TTS ( /tts [message] )
			...
			temp =
				msg.split(" 𐤟 ")[0] +
				" 𐤟 " +
				" tried to use an unrecognised command.";
			msg = temp;
		}
	}
	// Dynamic links and images
	else if (
		msg.split(" 𐤟 ")[1].substring(1, 8) == "http://" ||
		msg.split("𐤟")[1].substring(1, 9) == "https://" ||
		msg.split("𐤟")[1].substring(1, 7) == "ftp://"
	) {
